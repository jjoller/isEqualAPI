...
public class Sentence {
	...
	public Sentence(String sentence) throws IOException, ClassNotFoundException {

		// sentence not in DB
		this.setOrginalSentence(sentence);

		// check DB for similar querys
		MapDBStore mDB = MapDBStore.getInstance();
		Map<String, byte[]> stanfordSMap = mDB.getDb()
				.hashMap("stanfordSentence", Serializer.STRING, Serializer.BYTE_ARRAY).createOrOpen();

		if (stanfordSMap.get(sentence) != null) {
			this.setStanfordSentence((CoreMap) mDB.deserialize(stanfordSMap.get(sentence)));
		} else

		{
			stanfordSentence = StanfordUtility.processSentence(sentence).get(0);

			// store sentence to db
			stanfordSMap.put(sentence, mDB.serialize(stanfordSentence));
		}

		// Initialize groups
		this.groupes = new Integer[getSize()];
